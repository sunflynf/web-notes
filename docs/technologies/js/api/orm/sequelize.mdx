...
## 4. Using Sequelize with [Express](/docs/technologies/js/be-frameworks/express)
...
```ts title="src/routes/user.routes.ts"
...
## 5. Using Sequelize with [NestJS](/docs/technologies/js/be-frameworks/nest)
...
## 6. [Migrations](https://sequelize.org/docs/v6/other-topics/migrations/)

Use `sequelize-cli` for production schema changes:
...
- Use Plain Sequelize (`extends Model<ModelAttributes, ModelCreationAttributes> implements ModelAttributes`)
  ...
  - You're using **Express** and _don't need NestJS's dependency injection_.
